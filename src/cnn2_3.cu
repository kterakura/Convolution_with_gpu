...
__global__ void d_cnn(float *A, float *B, float *C, int i){
    ...
    tile_B[0] = B[0 + i*BSIZE_X*BSIZE_Y];
    tile_B[1] = B[1+ i*BSIZE_X*BSIZE_Y];
    tile_B[2] = B[2+ i*BSIZE_X*BSIZE_Y];
    tile_B[3] = B[3+ i*BSIZE_X*BSIZE_Y];
    tile_B[4] = B[4+ i*BSIZE_X*BSIZE_Y];
    tile_B[5] = B[5+ i*BSIZE_X*BSIZE_Y];
    tile_B[6] = B[6+ i*BSIZE_X*BSIZE_Y];
    tile_B[7] = B[7+ i*BSIZE_X*BSIZE_Y];
    tile_B[8] = B[8+ i*BSIZE_X*BSIZE_Y];
    ...
    C[(idy)*CSIZE_X+(idx) + i*CSIZE_X*CSIZE_Y] += tile_A1[tx]*tile_B[0];
    C[(idy)*CSIZE_X+(idx) + i*CSIZE_X*CSIZE_Y] += tile_A1[tx+1]*tile_B[1];
    C[(idy)*CSIZE_X+(idx) + i*CSIZE_X*CSIZE_Y] += tile_A1[tx+2]*tile_B[2];
    C[(idy)*CSIZE_X+(idx) + i*CSIZE_X*CSIZE_Y] += tile_A2[tx]*tile_B[3];
    C[(idy)*CSIZE_X+(idx) + i*CSIZE_X*CSIZE_Y] += tile_A2[tx+1]*tile_B[4];
    C[(idy)*CSIZE_X+(idx) + i*CSIZE_X*CSIZE_Y] += tile_A2[tx+2]*tile_B[5];
    C[(idy)*CSIZE_X+(idx) + i*CSIZE_X*CSIZE_Y] += tile_A3[tx]*tile_B[6];
    C[(idy)*CSIZE_X+(idx) + i*CSIZE_X*CSIZE_Y] += tile_A3[tx+1]*tile_B[7];
    C[(idy)*CSIZE_X+(idx) + i*CSIZE_X*CSIZE_Y] += tile_A3[tx+2]*tile_B[8];
}
...
int main(){
    ...
    float *h_B = (float *)malloc(BSIZE_X * BSIZE_Y * sizeof(float) * NUM_KERNEL);
    float *h_C = (float *)malloc(CSIZE_X * CSIZE_Y * sizeof(float) * NUM_KERNEL);
    float *ans_C = (float *)malloc(CSIZE_X * CSIZE_Y * sizeof(float) * NUM_KERNEL);
    ...
    CHECK(cudaMemcpy( d_B, h_B, BSIZE_X * BSIZE_Y * sizeof(float) * NUM_KERNEL, cudaMemcpyHostToDevice));
    for(int i=0; i<NUM_KERNEL; i++){
        //gpu
        d_cnn<<< grid, block, 0, streams[i] >>>(d_A, d_B, d_C, i);
    }
    CHECK(cudaMemcpy(ans_C, d_C, CSIZE_X * CSIZE_Y * sizeof(float) * NUM_KERNEL, cudaMemcpyDeviceToHost));
    cnn_time = cpuSecond() - cnn_start;
    printf("gpu time: %f\n", cnn_time);
